...
define_macro! {
    ...
    ///	external resource. This element is most commonly used to link to CSS, but is also used to
    ...
    /// (e.g., in syndication). Examples include: a forum post, a magazine or newspaper article, or
    /// a blog entry, a product card, a user-submitted comment, an interactive widget or gadget, or
    ...
    aside,
    /// The `<footer>` HTML element represents a footer for its nearest sectioning content or
    /// sectioning root element. A `<footer>` typically contains information about the author of
    ...
    header,
    /// The `<hgroup>` HTML element represents a heading and related content. It groups a single
    ...
    /// content or layout until styled in some way using CSS (e.g. styling is directly applied to
    ...
    /// must be used inside a dl element. It is usually followed by a dd element; however, multiple
    ...
    figcaption,
    /// The `<figure>` HTML element represents self-contained content, potentially with an optional
    /// caption, which is specified using the figcaption element. The figure, its caption, and its
    ...
    /// indentation, but HTML paragraphs can be any structural grouping of related content, such as
    ...
    /// boldface text, or the strong element to indicate that text is of special importance.
    b,
    ...
    data,
    /// The `<dfn>` HTML element is used to indicate the term being defined within the context of a
    /// definition phrase or sentence. The p element, the dt/dd pairing, or the section element
    ...
    /// However, `<s>` is not appropriate when indicating document edits; for that, use the del and
    ...
    /// like a div element, but div is a block-level element whereas a `<span>` is an inline
    /// element.
    span,
    ...
    /// attribute to translate dates into machine-readable format, allowing for better search
    ...
    u,
    /// The `<var>` HTML element represents the name of a variable in a mathematical expression or
    /// a programming context. It's typically presented using an italicized version of the current
    ...
    /// the browser may optionally break a line, though its line-breaking rules would not otherwise
    /// create a break at that location.
    wbr,
    /// The `<area>` HTML element defines an area inside an image map that has predefined clickable
    /// areas. An image map allows geometric areas on an image to be associated with Hyperlink.
    area,
    ...
    /// handle subtitles. The tracks are formatted in WebVTT format (.vtt files) — Web Video Text
    /// Tracks.
    track,
    ...
    picture,
    /// The `<portal>` HTML element enables the embedding of another HTML page into the current one
    ...
    svg,
    /// The top-level element in MathML is `<math>.` Every valid MathML instance must be wrapped in
    /// `<math>` tags. In addition you must not nest a second `<math>` element in another, but you
    ...
    tr,
    /// The `<button>` HTML element represents a clickable button, used to submit forms or anywhere
    ...
    /// widgets are available, depending on the device and user agent. The `<input>` element is one
    /// of the most powerful and complex in all of HTML due to the sheer number of combinations of
    ...
    details,
    /// The `<dialog>` HTML element represents a dialog box or other interactive component, such as
    ...
    /// of items (represented by li elements), each of these represent a link or other command that
    ...
    /// immediately when a page is loaded but may be instantiated subsequently during runtime using
    /// JavaScript.
    template,
}
